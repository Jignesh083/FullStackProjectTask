...
//   private handleTokenExpired(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
//     console.log("handleToken Expired called....");
//     if (!this.isRefreshing) {
//       this.isRefreshing = true;
//       this.refreshTokenSubject.next(null);
//       console.log('Refreshing token...');
...
//   private logoutAndRedirect(): Observable<any> {
//     console.log("logoutAndReirect called....");
//     this.authService.logout();
//     alert('Session expired. Redirecting to login.');
//     this.router.navigate(['/login']);
//     return throwError(() => new Error('Session expired'));
...
export class AuthInterceptor implements HttpInterceptor {
  ...
  private handleTokenExpired(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    console.log("handleTokenExpired called....");
    ...
    if (!this.isRefreshing) {
      ...
      const refreshToken = this.authService.getRefreshToken();  //  Fix: Ensure we get refresh token
      if (!refreshToken) {
        console.error('No refresh token found. Logging out...');
        this.isRefreshing = false;
        return this.logoutAndRedirect();
      ...
      return this.authService.refreshAccessToken().pipe(
        switchMap((response: any) => {
          ...
          this.authService.storeToken(response.accessToken);  //  Fix: Ensure new tokens are saved properly
          this.authService.storeRefreshToken(response.refreshToken);
          this.refreshTokenSubject.next(response.accessToken);

          return next.handle(this.addToken(req, response.accessToken)); //  Retry original request
        }),
        catchError((err) => {
          console.error('Token refresh failed:', err);
          this.isRefreshing = false;
      ...
      return this.refreshTokenSubject.pipe(
        filter(token => token !== null),
        take(1),
        switchMap(token => next.handle(this.addToken(req, token!)))  // Retry original request with new token
      );
    }
  }
